...
@interface SASToggleMessage ()
+ (SASToggleMessage *)fromMap:(NSDictionary *)dict;
- (NSDictionary *)toMap;
@end
@interface SASIsEnabledMessage ()
+ (SASIsEnabledMessage *)fromMap:(NSDictionary *)dict;
- (NSDictionary *)toMap;
@end

@implementation SASToggleMessage
+ (SASToggleMessage *)fromMap:(NSDictionary *)dict {
  SASToggleMessage *result = [[SASToggleMessage alloc] init];
...
@implementation SASIsEnabledMessage
+ (SASIsEnabledMessage *)fromMap:(NSDictionary *)dict {
...
- (nullable id)readValueOfType:(UInt8)type 
{
  switch (type) {
    case 128:     
      return [SASIsEnabledMessage fromMap:[self readValue]];
    
    case 129:     
      return [SASToggleMessage fromMap:[self readValue]];
...
{
  if ([value isKindOfClass:[SASIsEnabledMessage class]]) {
    [self writeByte:128];
    [self writeValue:[value toMap]];
  } else 
  if ([value isKindOfClass:[SASToggleMessage class]]) {
    [self writeByte:129];
    [self writeValue:[value toMap]];
  } else 
{
...
void FLTWakelockApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLTWakelockApi> *api) {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
          SASToggleMessage *arg_msg = args[0];
        FlutterError *error;
  ...
  }
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        FlutterError *error;
          SASIsEnabledMessage *output = [api isEnabledWithError:&error];
